...
{  
    ...
    link.rel  = 'stylesheet';
    link.type = 'text/css';
    link.href = 'sites/all/modules/chat/bundle/jquery/css/base/jquery-ui.css';
    link.media = 'all';
...
if (!document.getElementById(cssId)){
    ...
    link.rel  = 'stylesheet';
    link.type = 'text/css';
    link.href = 'sites/all/modules/chat/css/jquery.ui.chatbox.css';
    link.media = 'all';
    head.appendChild(link);
}
...
jQuery.when(
    $.cachedScript( "sites/all/modules/chat/bundle/io/build/iolib.min.js" ),
    $.cachedScript( "sites/all/modules/chat/build/chat.min.js" )

 ).done(function(){

     $.uiBackCompat = false;
     //accessing values those were saved with drupal setting
     var userobj = {'userid':Drupal.settings.chat.id,'name':Drupal.settings.chat.name,'img':Drupal.settings.chat.imageurl};
     var room = 'main-c-room';//ToDo:

     userdata = {
            'userid':Drupal.settings.chat.id,
            'sid':'212',
            'rid': Drupal.settings.chat.path,
            'authuser':Drupal.settings.chat.auth_user,
            'authpass':Drupal.settings.chat.auth_pass,
            'userobj': userobj,
            'fastchat_lasttime':'0',
     ...
     if(localStorage.getItem('init') != 'false'){
         ...
         if (localStorage.getItem(Drupal.settings.chat.sid) != null)  {
             displayChatHistory();
             vmstorage = JSON.parse(localStorage.getItem(Drupal.settings.chat.sid));
         }
         ...
         $(document).on("connectionclose", function(e){
             $("#user_list .inner_bt #usertab_icon").css({'background': 'url(sites/all/modules/chat/images/offline.png)no-repeat top left'});
         ...
         $(window).bind('beforeunload',function(){
                ...
                localStorage.setItem(Drupal.settings.chat.sid, data);
         });
   });
});
