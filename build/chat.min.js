...
$("#chatrm").hide()}
function displayChatHistory(){if(null!=localStorage.getItem(Drupal.settings.chat.sid)){var b=JSON.parse(localStorage.getItem(Drupal.settings.chat.sid));$.each(b,function(a,b){counter++;idList.push(a);$.each(b,function(b,c){1>b?(chatboxManager.addBox(a,{dest:"dest"+counter,title:"box"+counter,first_name:c.name}),chatboxManager.init({messageSent:function(a,b,c){$("#"+a).chatbox("option","boxManager").addMsg(b.name,c)}})):$("#"+a).chatbox("option").messageSent(a,c,c.msg)});"hidden"==localStorage.getItem(a)&&($("#cb"+a).hide(),$("#tabcb"+a).removeClass("ui-state-active"))})}}
